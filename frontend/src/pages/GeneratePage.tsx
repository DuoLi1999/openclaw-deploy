...
function ProgressStepList({ steps }: { steps: StreamProgress[] }) {
  // Deduplicate: keep latest event per nodeTitle
...
export function GeneratePage() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    // Switch to a tab that has results
  ...
  const handleCopy = useCallback(() => {
    const content = results[activeTab]?.result ?? "";
  ...
  const tabIsGenerating =
    isGenerating && !results[activeTab] && !!currentProgress?.length;
